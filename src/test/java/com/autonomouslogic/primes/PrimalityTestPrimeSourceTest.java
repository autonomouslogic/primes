...
public class PrimalityTestPrimeSourceTest {
	...
	void shouldWrapTrialDivision() {
		...
		var expected = new PrimeList().primeStream().filter(n -> n != 2).iterator();
	...
	void shouldWrapTrialDivisionAtOffset() {
		var source = new PrimalityTestPrimeSource(new TrialDivision(), 5000)
				.primeStream()
				.iterator();
		var expected = new PrimeList().primeStream().filter(n -> n > 5000).iterator();
		for (int i = 0; i < 1000; i++) {
			assertEquals(expected.nextLong(), source.nextLong());
		}
	}
